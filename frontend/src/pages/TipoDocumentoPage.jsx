...
import { getTiposDocumento, createTipoDocumento, updateTipoDocumento, deleteTipoDocumento } from '../services/tipoDocumentoService';
...
function TipoDocumentoPage() {
    ...
    const cargar = async () => {
        setCargando(true);
        setItems(await getTiposDocumento());
        setCargando(false);
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editandoId) {
                await updateTipoDocumento(editandoId, formData);
            } else {
    ...
    const handleEliminar = async (fila) => {
        ...
        try {
            await deleteTipoDocumento(fila.id);
            cargar();
        } catch (err) {
            console.error(err);
    ...
    if (cargando) return <p>Cargando tipos de documento...</p>;
    ...
    return (
        <div>
            ...
            <form onSubmit={handleSubmit}>
                ...
                <button type="submit">{editandoId ? 'Guardar cambios' : 'Agregar Tipo de Documento'}</button>
