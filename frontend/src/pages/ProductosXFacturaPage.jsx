...
import { getProductosXFactura, updateProductoXFactura, deleteProductoXFactura } from '../services/productosXFacturaService';
...
function ProductosXFacturaPage() {
    ...
    const cargar = async () => {
        setCargando(true);
        const [dataItems, dataProductos, dataPresentaciones, dataFacturas] = await Promise.all([
            getProductosXFactura(), getProductos(), getPresentaciones(), getFacturas()
        ]);
        setItems(dataItems);
    ...
    const referenciaFactura = (id) => {
        const f = facturas.find((factura) => factura.id === id);
        return f ? `#${f.id} — ${f.fecha ? f.fecha.slice(0, 10) : ''}` : `ID ${id}`;
    };

    const columnas = [
        { campo: 'id', titulo: 'ID' },
        { campo: 'id_factura', titulo: 'Factura', render: (fila) => referenciaFactura(fila.id_factura) },
    ...
    const handleEliminar = async (fila) => {
        if (!window.confirm('¿Eliminar este detalle de factura? Esto NO ajusta el stock automáticamente.')) return;
        try {
    ...
    if (cargando) return <p>Cargando detalle de facturas...</p>;

    return (
        <div>
            <h2>Detalle de Facturas — Correcciones</h2>
            <p style={{ color: 'var(--color-ink-soft)', marginTop: '-12px', marginBottom: '16px', fontSize: '13px' }}>
                Para registrar una factura nueva, usa "Nueva Factura" en el menú. Aquí solo se corrigen o eliminan líneas ya existentes.
            </p>
